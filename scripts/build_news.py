...
def card(x,cn):
 ...
 return f'''<article class="archive-story" id="{x['id']}" data-date="{x['published']}" data-topic="{x['topic']}" data-year="{x['published'][:4]}" aria-labelledby="title-{x['id']}"><div class="story-picture">{photo(x,cn)}</div><div class="story-content"><div class="story-meta"><span class="story-topic">{CATS[x['topic']][cn]}</span><span>{tr('Published','发布于',cn)} <time datetime="{x['published']}">{day(x['published'],cn)}</time></span></div><h2 id="title-{x['id']}">{title}</h2>{event}<p class="story-summary">{escape(field(x,'summary',cn))}</p><details class="story-details"><summary><span>{tr('Read story','阅读详情',cn)}</span>{SVG}</summary><div class="story-full">{archive_note}{body}<div class="story-gallery">{gallery}</div><div class="story-source-links"><a href="{escape(x['sourceUrl'],quote=True)}" target="_blank" rel="noopener">{source} <span aria-hidden="true">↗</span></a>{links}</div></div></details><div class="story-foot"><span>{origin}</span><a class="story-permalink" href="#{x['id']}" aria-label="{escape(tr('Link to: ','链接至：',cn)+field(x,'title',cn),quote=True)}">{tr('Permalink','本条链接',cn)}</a></div></div></article>'''
def main(cn):
...
for cn in [False,True]:
 ...
 for css in ['styles','refresh','controls','navigation','news']:head+=f'<link rel="stylesheet" href="{rel}{css}.css?v=20260921-4" />'
 for js in ['site','navigation','news']:head+=f'<script src="{rel}{js}.js?v=20260921-4" defer></script>'
